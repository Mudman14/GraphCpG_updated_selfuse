...
class CpGGraph(GNN):
    ...
    def __init__(self, gconv=RGCNConv, latent_dim=[32, 64, 128, 128, 64, 32],#[32, 32, 32, 32], 
                 ...
                 multiply_by=1, lr=1e-3, lr_decay_factor=.90, warmup_steps=1000): #注意：cell_num here 需要根据新的数据集adapt
    ...
    def test_step(self, batch, batch_idx):
        ...
        y = data.y.to(torch.float)
        y_hat = y_hat
        self.test_outputs = torch.stack((y_hat, y))
    ...
    def on_test_epoch_end(self):
        ...
        auroc_value = auroc(y_hat, y, task='binary')
